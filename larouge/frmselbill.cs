...
namespace larouge
{
    public partial class frmselbill : Form
    {
        static OleDbConnection con;
        static OleDbCommand cmd;
        static OleDbDataReader reader;
        private string appuserid;
        private int avilableunits = 0;
        ...
        public frmselbill(string userid)
        {
            InitializeComponent();
            appuserid = userid;
        }
        private void frmselbill_Load(object sender, EventArgs e)
        {
        ...
        {
            string selstr = "SELECT * FROM Qpoductforbill where productid > 0 ";
            try
            {
                OleDbConnection con = new OleDbConnection();
                con.ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["MyconnectionString"].ConnectionString;
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (dataGridView1.Rows[e.RowIndex].Cells[1].Value != null)
                {
                    ...
                    txtbaseprice.Text = dataGridView1.Rows[e.RowIndex].Cells[5].Value.ToString();
                    txtunitprice.Text = dataGridView1.Rows[e.RowIndex].Cells[6].Value.ToString();
                    txtpriceafteroffer.Text = dataGridView1.Rows[e.RowIndex].Cells[6].Value.ToString();
                    txtunits.Text = "0";
                    if (hasoffer(proid.ToString())==true )
                    {
                        btnoffer.BackColor = Color.Green;
                    }
                    else
                    {
        ...
        private bool hasoffer(string proid)
        {

            string selstr = "SELECT * FROM Toffers where productIdstore = " + proid;
            try
            {
                OleDbConnection con = new OleDbConnection();
                con.ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["MyconnectionString"].ConnectionString;
            ...
            {
                MessageBox.Show(ex.ToString(), "erorr at offertable");
            }
            return false;
        }
        ...
        private void txtunits_Leave(object sender, EventArgs e)
        {
            ...
            {
                if (Convert.ToInt16(txtunits.Text) > avilableunits)
                {
                    MessageBox.Show("خطأ في عدد الوحدات يجب الا يزيد عن المتوفر بالمخزن", "ادخل عدد الوحدات");
                }
                else
                {
        ...
        private void txtpriceafteroffer_Leave(object sender, EventArgs e)
        {
            decimal number = 0;
            if (decimal.TryParse(txtpriceafteroffer.Text.Trim(), out number) == true)
            {
                if (txtunits.TextLength  > 0)
                {
                    txtsumafteroffer.Text = Convert.ToString(Convert.ToInt16(txtunits.Text) * Convert.ToDecimal(txtpriceafteroffer.Text));
            ...
            {
                //not a number
                MessageBox.Show("برجاء ادخال سعر البيع بصورة صحيحة ارقام","خطأ سعر البيع");
                txtpriceafteroffer.Text = "";
            }
        ...
        {
            dataGridView2.Rows.Add(dataGridView2.Rows.Count+1, txtid.Text, txtproid.Text, txtponamesel.Text, txtunits.Text, txtpriceafteroffer.Text, txtsumafteroffer.Text,txtbaseprice.Text);
            ...
            try
            {
                if (cheackbilldata() == true)
                {
                    ...
                    //{
                        ...
                        string valuesstr = " VALUES (" + appuserid + "," + customerid + "," + 1 + ",'" + billid + "', # " + operationdate + "# ," + sumtotal + ")";
                        ...
                        if (sonuc > 0)
                        {
                            ...
                            {
                                savecustomerparts(customerid, remend);
                            }
                        }
        ...
        private void savebillitems(long operationid)
        {
            try
            {
                ...
                for (int rownum = 0; rownum < dataGridView2.Rows.Count; rownum++)
                {
                    ...
                    decimal baseprice = Convert.ToDecimal(dataGridView2.Rows[rownum].Cells[7].Value.ToString());

                    cmd = new OleDbCommand();
                    cmd.Connection = con;

                    upstr = "INSERT INTO Tbilldata (operationid,productid,numofunits,pricevalueofone,pricevalueofonebase)  ";
                    string valuesstr = " VALUES (" + operationid + "," + id + "," + avilableunits + "," + selprice + "," + baseprice+")";
                    upstr = upstr + valuesstr;
                    cmd.CommandText = upstr;
      ...
      //          con.Close();
        ...
        private long getmaxid(string tblname, string colname)
        {
            long newid = 0;
        ...
        private bool cheackbilldata()
        {
            ...
            {
                MessageBox.Show("يجب ادخال منتجات", "خطأ اضافة فاتورة بيع", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            if (Convert.ToDecimal(txtsumtotal.Text) <= 0)
            {
                MessageBox.Show("يجب ادخال قيمة مدفوعة", "خطأ اضافة فاتورة بيع", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }

            if (cmbcustomer.Text == "")
            {
                MessageBox.Show("يجب اختيار عميل", "خطأ اضافة فاتورة بيع", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            return true;
        }
        ...
        private void btnoffer_Click(object sender, EventArgs e)
        {
            ...
            frmoffers newMDIChild = new frmoffers(txtid.Text);
