...
namespace larouge
{
    public partial class frmtypes : Form
    {
        ...
        {
            fillgridfromtable(dataGridView1, "Tproducttypes", 3, "");
        }
            ...
            try
            {
                ...
                using (cmd = new OleDbCommand(selstr, con))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        dg.Rows.Add(rownum.ToString(), reader["producttypeid"].ToString(), reader["producttype"].ToString(), reader["isactive"].ToString());
                        rownum++;
                    }
        ...
        {
            savetablefromgrid(dataGridView1, "Tproducttypes");
        }
            ...
            try
            {
                for (int rowi = 0; rowi < dg.Rows.Count - 1; rowi++)
                {
                    ...
                    if (dg.Rows[rowi].Cells[1].Value != null)
                    {
                        ...
                        cmd.Connection = con;
                        con.Open();
                        upstr = "UPDATE " + tbname + " SET  isactive=" + isactive + " ,producttype='" + namepro + "' where producttypeid = " + proid;
                    ...
                    else
                    {
                        ...
                        cmd.Connection = con;
                        con.Open();
                        upstr = "insert into " + tbname + "( isactive,producttype) values (" + isactive + " ,'" + namepro + "')";
        ...
        private void btndel_Click(object sender, EventArgs e)
        {
            if (selgrid >= 0)
            {
                ...
                if (MessageBox.Show("هل تريد حذف هذا النوع من الوحدات", "حذف نوع", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                {
                    deleterecordid("Tproducttypes", " producttypeid =" + id);
                }
                fillgridfromtable(dataGridView1, "Tproducttypes", 4, "");
            }
        }
